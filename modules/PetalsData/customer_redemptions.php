<?php
$header['pageTitle'] = "Customer Redemptions - Petals Data";
...
try {
    ...
    // Query to get redemption metrics per customer using the same logic as the website
    ...
    $sql = "
        ...
        LEFT JOIN (
            ...
            FROM company_loyalty_points clp
            GROUP BY clp.company_id
        ) lp_stats ON co.id = lp_stats.company_id
        LEFT JOIN (
            SELECT
                clp.company_id,
                SUM(clp.points_earned - clp.points_redeemed) AS available_points
            FROM company_loyalty_points clp
            WHERE clp.point_status_id = (SELECT config_value FROM settings WHERE config_key = 'loyalty_available_status_id')
            GROUP BY clp.company_id
        ) ap_stats ON co.id = ap_stats.company_id
        LEFT JOIN (
            SELECT
                clp.company_id,
                SUM(clp.points_earned) AS pending_points
            FROM company_loyalty_points clp
            WHERE clp.point_status_id = (SELECT config_value FROM settings WHERE config_key = 'loyalty_pending_status_id')
            GROUP BY clp.company_id
        ) pp_stats ON co.id = pp_stats.company_id
        LEFT JOIN (
            SELECT
                clp.company_id,
                SUM(clp.points_earned) AS canceled_points
            FROM company_loyalty_points clp
            WHERE clp.point_status_id = (SELECT config_value FROM settings WHERE config_key = 'loyalty_canceled_status_id')
            GROUP BY clp.company_id
        ) cp_stats ON co.id = cp_stats.company_id
        LEFT JOIN (
            SELECT
                clp.company_id,
                SUM(clp.points_earned) AS expired_points
            FROM company_loyalty_points clp
            WHERE clp.point_status_id = (SELECT config_value FROM settings WHERE config_key = 'loyalty_expired_status_id')
            GROUP BY clp.company_id
        ) ep_stats ON co.id = ep_stats.company_id
        LEFT JOIN (
            SELECT
                crp.company_id,
                SUM(crp.dollars) AS claimed_rewards,
                SUM(crp.points) AS points_redeemed
            FROM company_redeemed_points crp
            WHERE DATE(crp.created_at) >= ?
              AND DATE(crp.created_at) <= ?
            GROUP BY crp.company_id
        ) cr_stats ON co.id = cr_stats.company_id
        WHERE co.status = 1
...
<div class="container mt-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Customer Redemptions</li>
        </ol>
    </nav>
    
    <div class="row mb-3">
        <div class="col-12">
            <h2><i class="bi bi-people-fill me-2"></i>Customer Redemptions</h2>
            <p class="text-muted">
                Loyalty points breakdown by customer (1000 petals = $25, earn 1 petal per dollar spent)
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header text-white d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                    <h5 class="mb-0"><i class="bi bi-table me-2"></i>Redemptions by Customer</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0" id="redemptionsTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td data-order="<?php echo htmlspecialchars($row['company_code']); ?>"><?php echo htmlspecialchars($row['company_code']); ?></td>
                                    <td data-order="<?php echo htmlspecialchars($row['company_name']); ?>"><strong><?php echo htmlspecialchars($row['company_name']); ?></strong></td>
...
<script>
// Initialize DataTables for sorting - wait for both jQuery and DataTables
(function() {
    function initDataTable() {
        if (typeof jQuery !== 'undefined' && jQuery.fn.DataTable && jQuery('#redemptionsTable').length) {
            ...
            jQuery('#redemptionsTable').DataTable({
                order: [[5, 'desc']], // Default sort by Points Redeemed (column 5) descending
                pageLength: 25,
                lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
                columnDefs: [
                    { 
                        targets: [3, 4, 5, 6], // Numeric columns (Total Points Earned, Available, Points Redeemed, Claimed Rewards)
                        type: 'num'
                    }
                ],
    ...
    try {
        ...
        rowArray.forEach(row => {
            ...
            cells.forEach((cell, index) => {
                ...
                // Get branch name from column index 2
                if (index === 2) {
                    branchName = cellText;
                }
        ...
        const headers = ['Company Code', 'Company Name', 'Branch', 'Total Points Earned', 'Available Points', 'Points Redeemed', 'Claimed Rewards'];
        ...
        let grandAvailablePoints = 0;
        let grandPointsRedeemed = 0;
        let grandClaimedRewards = 0;
        ...
        sortedBranches.forEach(branchName => {
            ...
            branchRows.forEach(rowData => {
                ...
                grandTotalPoints += parseFloat((rowData[3] || '0').toString().replace(/,/g, '')) || 0;
                grandAvailablePoints += parseFloat((rowData[4] || '0').toString().replace(/,/g, '')) || 0;
                grandPointsRedeemed += parseFloat((rowData[5] || '0').toString().replace(/,/g, '')) || 0;
                grandClaimedRewards += parseFloat((rowData[6] || '0').toString().replace(/,/g, '')) || 0;
            });
        });
        ...
        allBranchesData.push([
            ...
            '',
            'Grand Total',
            Math.round(grandTotalPoints).toString(),
            Math.round(grandAvailablePoints).toString(),
            Math.round(grandPointsRedeemed).toString(),
            grandClaimedRewards.toFixed(2)
        ]);
        ...
        sortedBranches.forEach(branchName => {
            ...
            let branchTotalPoints = 0;
            let branchAvailablePoints = 0;
            let branchPointsRedeemed = 0;
            let branchClaimedRewards = 0;
            
            branchRows.forEach(rowData => {
                // Remove commas and parse numbers
                branchTotalPoints += parseFloat((rowData[3] || '0').toString().replace(/,/g, '')) || 0;
                branchAvailablePoints += parseFloat((rowData[4] || '0').toString().replace(/,/g, '')) || 0;
                branchPointsRedeemed += parseFloat((rowData[5] || '0').toString().replace(/,/g, '')) || 0;
                branchClaimedRewards += parseFloat((rowData[6] || '0').toString().replace(/,/g, '')) || 0;
            });
            ...
            exportData.push([
                ...
                '',
                'Total',
                Math.round(branchTotalPoints).toString(),
                Math.round(branchAvailablePoints).toString(),
                Math.round(branchPointsRedeemed).toString(),
                branchClaimedRewards.toFixed(2)
            ]);
        ...
        const filename = `Customer_Redemptions_${fromDate}_to_${toDate}.xlsx`;
